...
p1 <- p1 + risk_binary(Y ~ S.1 * Z, D = 50) + ps_estimate()
p1 <- p1 + risk_binary(Y ~ S.1 * Z, D = 50, risk = risk.probit) + ps_estimate()
p1 <- p1 + risk_binary(Y ~ bs(S.1, df = 2, degree = 1) * Z, D = 50, risk = risk.probit) + ps_estimate()
